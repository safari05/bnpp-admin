        ...
        <div class="intro-y box mt-5" id="daftar-kepeg">
            <div class="flex items-center px-5 py-3 border-b border-gray-200 dark:border-dark-5">
                ...
                <div class="font-medium text-gray-700 dark:text-gray-500">
                    <a href="{{url('desa/kepeg')}}/{{$desa->id}}/create">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('body').on('click', '.btn-edit-kepeg', function(){
            window.location.replace("{{url('desa/kepeg').'/'.$desa->id.'/edit' }}/"+$(this).data('id'));
        })
    })
    ...
    async function initTableKepeg() {
        table_aset = await $('#list-kepeg').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ url('desa/kepeg').'/'.$desa->id.'/list' }}',
                data : function(d){
                    d.q = $('#name').val();
                }
            },
    ...
    function chartPeg(){
        var chart_aset;
        $.getJSON('{{url('desa/kepeg')}}/{{$desa->id}}/c/peg', res=>{
    ...
    function chartASN(){
        var chart_aset;
        $.getJSON('{{url('desa/kepeg')}}/{{$desa->id}}/c/asn', res=>{
    ...
    function chartLembaga(){
        var chart_aset;
        $.getJSON('{{url('desa/kepeg')}}/{{$desa->id}}/c/lembaga', res=>{
