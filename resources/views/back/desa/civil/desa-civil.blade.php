...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex">
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Penduduk</a>
</div>
@endsection
    ...
    <div class="col-span-12 lg:col-span-8 xxl:col-span-9">
        ...
        <div class="intro-y box mt-5" id="detail-sipil">
            <div class="flex items-center px-5 py-3 border-b border-gray-200 dark:border-dark-5">
                ...
                <div class="font-medium text-gray-700 dark:text-gray-500">
                    <a href="{{url('desa/sipil')}}/{{$desa->id}}/detail/create">
        ...
        <div class="intro-y box mt-5" id="daftar-sipil">
            <div class="flex items-center px-5 py-3 border-b border-gray-200 dark:border-dark-5">
                ...
                <div class="font-medium text-gray-700 dark:text-gray-500">
                    <a href="{{url('desa/sipil')}}/{{$desa->id}}/ages/create">
                        <button type="button" class="button button--sm block bg-theme-1 text-white">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('body').on('click', '.btn-edit-sipil', function(){
            window.location.replace("{{url('desa/sipil').'/'.$desa->id.'/edit' }}/"+$(this).data('id'));
        })
    })

    function getDetailSipil(){
        $.getJSON('{{url('desa/sipil')}}/{{$desa->id}}/data-detail', res=>{
            if(res.status == 200){
    ...
    async function initTableSipil() {
        table_aset = await $('#list-sipil').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ url('desa/sipil').'/'.$desa->id.'/list' }}',
                data : function(d){
                    d.q = $('#name').val();
                }
            },
    ...
    function chartGender(){
        var chart_gender;
        $.getJSON('{{url('desa/sipil')}}/{{$desa->id}}/c/gender', res=>{
    ...
    function chartUmur(){
        var chart_age;
        $.getJSON('{{url('desa/sipil')}}/{{$desa->id}}/c/age', res=>{
