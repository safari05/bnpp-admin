...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex">
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="{{route('desa.index')}}">Kependudukan</a>
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="{{url('desa/aset')}}/{{$desa->id}}">Umur</a>
...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">
        {{ucwords($mode)}} Detail Umur Kependudukan Desa {{ucwords(strtolower($desa->name))}}
    </h2>
</div>
...
<script>
    ...
    function submitPenduduk(){
        Swal.fire({
            ...
            text: 'Anda akan menginput Jumlah Penduduk Desa berdasarkan Umur. Lanjutkan?',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url:'{{route('desa.sipil.store.ages')}}',
                    method:'post',
                    data: data,
                    success:res=>{
                        if (res.status == 200) {
                            showSuccess(res.msg, ()=>{
                                window.location.replace('{{url('desa/sipil')}}/{{$desa->id}}');
                            });
                        } else {
                            showError(res.msg);
                        }
    ...
    function updatePenduduk(){
        Swal.fire({
            title: 'Yakin data sudah benar?',
            text: 'Anda akan mengubah Jumlah Penduduk Desa berdasarkan Umur. Lanjutkan?',
            showCancelButton: true,
            confirmButtonText: 'Lanjutkan',
            cancelButtonText: 'Tutup',
        }).then((result) => {
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url:'{{route('desa.sipil.update.ages')}}',
                    method:'post',
                    data: data,
                    success:res=>{
                        if (res.status == 200) {
