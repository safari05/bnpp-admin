...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex">
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="{{url('desa/aset')}}/{{$desa->id}}">Aset</a>
...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">
        {{ucwords($mode)}} Aset Desa {{ucwords(strtolower($desa->name))}}
    </h2>
</div>
...
<script>
    ...
    $(document).ready(function(){
        $.getJSON('{{route('desa.aset.src-auto')}}', data=>{
            $( "#nama" ).autocomplete({
                source: data
            });
        })
    ...
    function submitItem(){
        Swal.fire({
            ...
            text: 'Anda akan menginput Aset Desa dengan data tersebut. Lanjutkan?',
            ...
            if (result.isConfirmed) {
                ...
                $.post('{{route('desa.aset.store')}}', data, res=>{
                    if (res.status == 200) {
                        showSuccess(res.msg, ()=>{
    ...
    function updateItem(){
        Swal.fire({
            title: 'Yakin data sudah benar?',
            text: 'Anda akan mengubah Aset Desa dengan data tersebut. Lanjutkan?',
            showCancelButton: true,
            confirmButtonText: 'Lanjutkan',
            cancelButtonText: 'Tutup',
        }).then((result) => {
            ...
            if (result.isConfirmed) {
                ...
                $.post('{{route('desa.aset.update')}}', data, res=>{
                    if (res.status == 200) {
                        showSuccess(res.msg, ()=>{
                            window.location.replace('{{url('desa/aset')}}/{{$desa->id}}');
                        });
