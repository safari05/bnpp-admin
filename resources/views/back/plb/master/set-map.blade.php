...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex">
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="{{url('plb/master/detail')}}/{{$plb->idplb}}">Lokasi</a>
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">{{ucwords($mode)}} Lokasi Pos Lintas Batas</a>
</div>
@endsection
...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">
        {{ucwords($mode)}} Lokasi Pos Lintas Batas {{ucwords(strtolower($plb->nama_plb))}}
    </h2>
</div>
        ...
        <div class="intro-y box mt-5">
            <div class="flex items-center px-5 py-3 border-b border-gray-200 dark:border-dark-5">
                <h2 class="font-medium text-base mr-auto">
                    Pilih Lokasi Pos Lintas Batas
                    <br>
                    <small>Geser pin ke lokasi Pos Lintas Batas</small>
                </h2>
...
<script>
    ...
    function initMapDesa(){
        ...
        specialMarker = L.marker(coord, {
            icon:plbicon,
            draggable:true,
        }).bindPopup('<button type="button" class="button button--sm block bg-theme-1 text-white" onclick="submitMap()">Simpan Lokasi PLB</button>').addTo(map);
    ...
    function submitMap(){
        Swal.fire({
            ...
            text: 'Pastikan Koordinat Pos Lintas Batas sudah benar. Lanjutkan?',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url:'{{route('plb.master.pin.store')}}',
                    method:'post',
                    data: data,
                    success:res=>{
                        if (res.status == 200) {
                            showSuccess(res.msg, ()=>{
                                window.location.replace('{{url('plb/master/detail')}}/{{$plb->idplb}}');
                            });
                        } else {
                            showError(res.msg);
                        }
