            ...
            <div class="grid grid-cols-12 gap-6 mt-5">
                <div class="col-span-12 intro-y">
                    <div class="grid grid-cols-12 gap-2">
                        <input type="text" class="input w-full border col-span-10" placeholder="Nama Jenis Staff Operasional" name="name" id="name" tabindex="1">
    ...
    <div class="modal__content p-10 text-center">
        <div class="grid grid-cols-12 gap-6">
            <div class="col-span-12">
                <h3>Tambah Jenis Staff Operasional</h3>
            </div>
            <div class="col-span-12 text-center">
                <input type="text" class="input w-full border" placeholder="Nama Jenis Staff Operasional" name="input-nama" id="input-nama">
            </div>
...
<script>
    ...
    function initTable() {
        table = $('#list-prov').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('master.kep.operasional.list') }}',
                data : function(d){
                    d.q = $('#name').val();
                }
            },
    ...
    function tambahAset(){
        ...
        $('#btn-simpan-aset').on('click', function(){
            ...
            console.log(data);
            $.post('{{route('master.kep.operasional.store')}}', data, res=>{
                if (res.status == 200) {
                    refreshTable();
                    showSuccess(res.msg);
                } else {
    ...
    function hapusAset(id){
        Swal.fire({
            title: 'Hapus Jenis Staff Operasional',
            text: 'Jenis Staff Operasional yang dihapus tidak dapat dikembalikan. Lanjutkan?',
            ...
            if (result.isConfirmed) {
                ...
                $.post('{{url('kepdata/operasional/delete')}}/'+id, data, res=>{
                    if (res.status == 200) {
                        refreshTable();
                        showSuccess(res.msg);
                    } else {
