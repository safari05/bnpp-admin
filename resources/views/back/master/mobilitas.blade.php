...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex">
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="" class="breadcrumb--active">Mobilitas</a>
</div>
@endsection
@section('content')
            ...
            <div class="grid grid-cols-12 gap-6 mt-5">
                <div class="col-span-12 intro-y">
                    <div class="grid grid-cols-12 gap-2">
                        <input type="text" class="input w-full border col-span-10" placeholder="Nama Mobilitas" name="name" id="name">
    ...
    <div class="modal__content p-10 text-center">
        <div class="grid grid-cols-12 gap-6">
            ...
            <div class="col-span-12 text-center">
                <input type="text" class="input w-full border" placeholder="Nama Mobilitas" name="input-nama" id="input-nama">
            </div>
...
<script>
    ...
    function initTable() {
        table = $('#list-prov').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('master.mobilitas.list') }}',
                data : function(d){
                    d.q = $('#name').val();
                }
            },
    ...
    function tambahAset(){
        ...
        $('#btn-simpan-aset').on('click', function(){
            ...
            console.log(data);
            $.post('{{route('master.mobilitas.store')}}', data, res=>{
                if (res.status == 200) {
                    refreshTable();
                    showSuccess(res.msg);
                } else {
    ...
    function hapusAset(id){
        Swal.fire({
            title: 'Hapus Mobilitas',
            text: 'Mobilitas yang dihapus tidak dapat dikembalikan. Lanjutkan?',
            ...
            if (result.isConfirmed) {
                ...
                $.post('{{url('master/mobilitas/delete')}}/'+id, data, res=>{
                    if (res.status == 200) {
                        refreshTable();
                        showSuccess(res.msg);
                    } else {
