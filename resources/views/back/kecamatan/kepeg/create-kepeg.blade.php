...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex">
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="{{url('kec/aset')}}/{{$kec->id}}">Kepegawaian</a>
...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">
        {{ucwords($mode)}} Kepegawaian Kecamatan {{ucwords(strtolower($kec->name))}}
    </h2>
</div>
...
<script>
    ...
    function submitPegawai(){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url:'{{route('kec.kepeg.store')}}',
                    method:'post',
                    data: data,
                    success:res=>{
                        if (res.status == 200) {
                            showSuccess(res.msg, ()=>{
                                window.location.replace('{{url('kecamatan/kepeg')}}/{{$kec->id}}');
                            });
                        } else {
                            showError(res.msg);
                        }
    ...
    function updatePegawai(){
        Swal.fire({
            title: 'Yakin data sudah benar?',
            text: 'Anda akan mengubah Data Pegawai Kecamatan dengan data tersebut. Lanjutkan?',
            showCancelButton: true,
            confirmButtonText: 'Lanjutkan',
            cancelButtonText: 'Tutup',
        }).then((result) => {
