    ...
    <div class="col-span-12 lg:col-span-8 xxl:col-span-9">
        ...
        <div class="intro-y box mt-5" id="daftar-aset">
            <div class="flex items-center px-5 py-3 border-b border-gray-200 dark:border-dark-5">
                ...
                <div class="font-medium text-gray-700 dark:text-gray-500">
                    <a href="{{url('kecamatan/aset')}}/{{$kec->id}}/create">
        ...
        <div class="intro-y box mt-5" id="daftar-mobil">
            <div class="flex items-center px-5 py-3 border-b border-gray-200 dark:border-dark-5">
                ...
                <div class="font-medium text-gray-700 dark:text-gray-500">
                    <a href="{{url('kecamatan/mobilitas')}}/{{$kec->id}}/create">
                        <button type="button" class="button button--sm block bg-theme-1 text-white">
                            Tambah Mobilitas
                        </button>
                    </a>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('body').on('click', '.btn-edit-aset', function(){
            window.location.replace("{{url('kecamatan/aset').'/'.$kec->id.'/edit' }}/"+$(this).data('id'));
        })

        $('body').on('click', '.btn-edit-mobil', function(){
            window.location.replace("{{url('kecamatan/mobilitas').'/'.$kec->id.'/edit' }}/"+$(this).data('id'));
        })
    ...
    async function initTableAset() {
        table_aset = await $('#list-aset').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ url('kecamatan/aset').'/'.$kec->id.'/list' }}',
                data : function(d){
                    d.q = $('#name').val();
                }
            },
    ...
    async function initTableMobil() {
        table_mobil = await $('#list-mobil').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ url('kecamatan/mobilitas').'/'.$kec->id.'/list' }}',
                data : function(d){
                    d.q = $('#name').val();
                }
            },
    ...
    function chartAset(){
        var chart_aset;
        $.getJSON('{{url('kecamatan/aset')}}/{{$kec->id}}/chart', res=>{
    ...
    function chartMobilitas(){
        var chart_aset;
        $.getJSON('{{url('kecamatan/mobilitas')}}/{{$kec->id}}/chart', res=>{
