...
<div class="-intro-x breadcrumb mr-auto hidden sm:flex">
    ...
    <i data-feather="chevron-right" class="breadcrumb__icon"></i> <a href="{{url('kec/aset')}}/{{$kec->id}}">Mobilitas</a>
...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">
        {{ucwords($mode)}} Mobilitas Kecamatan {{ucwords(strtolower($kec->name))}}
    </h2>
</div>
        ...
        <div class="intro-y box mt-5">
            ...
            <div class="p-5">
                    <form action="" enctype="multipart/form-data" id="form-mobil">
                        <div class="grid grid-cols-12 gap-6">
                        @csrf
                        <input type="hidden" name="id" id="id" value="{{$kec->id}}">
                        @if($mode == 'edit')
                            @method('put')
...
<script>
    ...
    $(document).ready(function(){
        $.getJSON('{{route('kec.mobilitas.src-auto')}}', data=>{
            $( "#nama" ).autocomplete({
                source: data
            });
        })
    ...
    function submitItem(){
        Swal.fire({
            ...
            text: 'Anda akan menginput Mobilitas Kecamatan dengan data tersebut. Lanjutkan?',
    ...
    function updateItem(){
        Swal.fire({
            title: 'Yakin data sudah benar?',
            text: 'Anda akan mengubah Mobilitas Kecamatan dengan data tersebut. Lanjutkan?',
            showCancelButton: true,
            confirmButtonText: 'Lanjutkan',
            cancelButtonText: 'Tutup',
        }).then((result) => {
            ...
            if (result.isConfirmed) {
                let formData = new FormData($('#form-mobil')[0]);
                $.ajax({
                    url:'{{route('kec.mobilitas.update')}}',
                    method:'post',
                    data: formData,
                    contentType: false,
                    processData: false,
