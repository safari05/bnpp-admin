...
class DesaKepegController extends Controller
{
    ...
    public function listKepeg($id){
        ...
        return \Yajra\DataTables\Facades\DataTables::of($prov)
            ...
            ->addColumn('aksi', function(DesaKepegawaian $data){
    ...
    public function create($id){
        ...
        if (Auth::user()->idrole != 5 || @Auth::user()->kecamatan->idkecamatan == @$desa->kecamatanid) {
            ...
            return view('back.desa.kepeg.create-kepeg', $data);
        }else{
            return back()->with('error', 'Anda tidak memiliki akses ke halaman tersebut!');
        }
    }
    ...
    public function store(Request $request){
        ...
        if(!$valid->fails()){
            ...
            $kepeg_exist = DesaKepegawaian::where([
                ['id', $input['id']],
                ['jenis_asn', $input['asn']],
                ['operasional', $input['staf']],
                ['kelembagaan', $input['lembaga']]
            ...
            try {
                ...
                if (empty($kepeg_exist)) {
                    DB::table('desa_kepegawaian')->insert($data_kepeg);
                }else{
                    DB::table('desa_kepegawaian')->where([
                        ['id', $input['id']],
                        ['jenis_asn', $input['asn']],
                        ['operasional', $input['staf']],
                        ['kelembagaan', $input['lembaga']]
            ...
            if($oke){
                return response()->json([
                    'status'=>200,
                    'msg'=>'Berhasil menambahkan Pegawai Desa',
                ]);
            }else{
                return response()->json([
                    'status'=>500,
                    'msg'=>'Gagal menambahkan Pegawai Desa',
                ]);
            }
    ...
    public function update(Request $request){
        ...
        if(!$valid->fails()){
            ...
            if($oke){
                return response()->json([
                    'status'=>200,
                    'msg'=>'Berhasil mengubah Jumlah Pegawai Desa',
                ]);
            }else{
                return response()->json([
                    'status'=>500,
                    'msg'=>'Gagal mengubah Jumlah Pegawai Desa',
                ]);
            }
